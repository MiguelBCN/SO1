...
typedef struct {
    unsigned long int passenger_count[1000];
    unsigned long int trip_time_in_secs[1000];
}block;
...
void productor(int fd[2], int ppid, char* filename){
    ...
    do{
        ...
        while (idx < N && fgets(line, sizeof(line), file)){
            ...
            if (!invalid) {
                data.passenger_count[idx]= value_col8; 
                data.trip_time_in_secs[idx]= value_col9;
                //printf("num_elements=%d passenger_count=%ld trip_time_in_secs=%ld\n", idx, data.passenger_count[idx], data.trip_time_in_secs[idx]);
                idx++;
            }
        }
...
void consumidor(int fd[2], int ppid){
    ...
    do{
      ...
      for(i=0; i<idx; i++){
      	pc+=data.passenger_count[i];
      	tt+=data.trip_time_in_secs[i];
      	num_elements++;
      }
